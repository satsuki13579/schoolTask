...
class ThreeJSTest {
    ...
    private ctrl_sphS: THREE.Mesh
    private ctrl_sph1: THREE.Mesh
    private ctrl_sph2: THREE.Mesh
    private ctrl_sphE: THREE.Mesh
...
//        var gui = new dat.GUI();
        ...
        gui.add(this.controls, 'ctrl1x', -100, 100).onChange((e:number)=>{
            this.controls.ctrl1x = e;
            //ここに値が変わったときの動作を追記
            this.renewGeo1();
        });
        gui.add(this.controls, 'ctrl1y', -100, 100).onChange((e:number)=>{
            this.controls.ctrl1y = e;
            //ここに値が変わったときの動作を追記
            this.renewGeo1();
        });
        gui.add(this.controls, 'ctrl1z', -100, 100).onChange((e:number)=>{
            this.controls.ctrl1z = e;
            //ここに値が変わったときの動作を追記
            this.renewGeo1();
        });
        gui.add(this.controls, 'ctrl2x', -100, 100).onChange((e:number)=>{
            this.controls.ctrl2x = e;
            //ここに値が変わったときの動作を追記
            this.renewGeo1();
        });
        gui.add(this.controls, 'ctrl2y', -100, 100).onChange((e:number)=>{
            this.controls.ctrl2y = e;
            //ここに値が変わったときの動作を追記
            this.renewGeo1();
        });
        gui.add(this.controls, 'ctrl2z', -100, 100).onChange((e:number)=>{
